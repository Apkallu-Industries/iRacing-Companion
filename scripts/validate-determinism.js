...
const activeProjection = {
  ...
  ontologyEvents: [
    {
      ...
      narrativeDescription:
        "Front axle brake pressure exceeded 82% threshold target under trail deceleration.",
    ...
    },
    {
      ...
      triggerValue: 12.8,
      narrativeDescription:
        "Rear driven exit wheel speed mismatched by more than 12% under re-application.",
  ...
  provenRecommendations: [
    {
      ...
      proposedAction:
        "Soften rear rebound dampers by -1 click to recover lateral tractive footprint.",
  ...
  sessionNarrative: {
    ...
    keyMechanicalNarrative:
      "Chassis decay stable. Heavy entry trail lockups observed at Turn 8 under downforce rake.",
    physicsTruthBoundary: { sourceTypes: ["behavioral_model"] },
  },
  driverTraits: {
    cornerEntryRotationPreference: "medium",
...
console.log(
  "\x1b[36m  PIT WALL CANONICAL STATE HASHING & REGRESSION TEST FIREWALL RUNNER     \x1b[0m",
);
console.log(
  "\x1b[36m========================================================================\x1b[0m",
);
    ...
    if (fieldName === "activeContext") {
      ...
      expected.forEach((evt) => {
        ...
        if (!matching) {
          ...
          console.log(
            `    ❌ Confidence mismatch on \x1b[31m${evt.eventType}\x1b[0m: Golden = ${evt.priority.confidence}, Active = ${matching.priority.confidence}`,
          );
        }
      });
      ...
      expected.forEach((rec) => {
        ...
        if (!matching) {
          console.log(
            `    ❌ Missing expected Recommendation: \x1b[31m${rec.proposedAction}\x1b[0m`,
          );
        } else if (matching.confidenceRating !== rec.confidenceRating) {
          console.log(
            `    ❌ Confidence mismatch on Setup Compromise: Golden = ${rec.confidenceRating}%, Active = ${matching.confidenceRating}%`,
          );
        }
      });
      ...
      Object.keys(expected).forEach((k) => {
        if (expected[k] !== current[k]) {
          console.log(
            `    ❌ Driver DNA trait \x1b[31m${k}\x1b[0m mismatch: Golden = "${expected[k]}", Active = "${current[k]}"`,
          );
        }
      });
    } else {
...
auditField(
  "provenRecommendations",
  activeProjection.provenRecommendations,
  goldenData.provenRecommendations,
);
...
console.log(
  "\x1b[41m\x1b[37m  FAILURE  \x1b[0m \x1b[31mRegression firewall tripped. Check your heuristic adjustments!\x1b[0m",
);
console.log(
  "\x1b[36m========================================================================\x1b[0m\n",
);
...
writeReport(
  "FAILURE",
  `Tripped regression firewall. Found ${driftCount} drift zones: ${driftViolations.join("; ")}`,
  durationMs,
);
process.exit(1);
