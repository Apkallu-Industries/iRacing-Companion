      ...
      <section className="relative overflow-hidden hairline-b">
        ...
        <div className="mx-auto max-w-5xl px-6 py-20">
          ...
          <h1 className="mt-4 text-4xl font-semibold leading-[1.05] tracking-tight md:text-6xl">
            From <span className="text-primary">binary tick stream</span>
            <br />
            to a workbench, in seconds.
          </h1>
          <p className="mt-6 max-w-2xl text-lg text-muted-foreground">
            Pit Wall reads iRacing's native <code className="font-mono text-foreground">.ibt</code>{" "}
            telemetry format directly in your browser. No plugins, no installs, no upload pipeline
            waiting on a server. The same data your sim writes to disk — parsed locally, indexed,
            and rendered as a MoTeC-style analysis workspace.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6 py-20 md:grid md:grid-cols-2 md:gap-16">
          <div>
            ...
            <h3 className="mt-2 text-3xl font-semibold tracking-tight">
              A binary log of every tick.
            </h3>
            <p className="mt-6 text-muted-foreground">
              An iRacing telemetry file is a fixed-layout binary stream: a 48-byte IRSDK header, a
              table of variable definitions, the session-info YAML, then a tightly packed array of
              tick records — one per simulator frame at 60 Hz (or 360 Hz for high-rate sessions).
            </p>
            <p className="mt-4 text-muted-foreground">
              ...
              its byte offset within a tick record, and its array count. Pit Wall materializes each
              variable into a typed array so any channel can be plotted instantly without a
              re-parse.
            </p>
          </div>
...
└─────────────────────────────────────────────┘`}
      ...
      <section className="mx-auto max-w-7xl px-6 py-20">
        ...
        <h3 className="mt-2 text-3xl font-semibold tracking-tight">
          Four panes, one synchronized cursor.
        </h3>
        ...
        <div className="mx-auto max-w-7xl grid gap-px overflow-hidden rounded-sm bg-border px-6 py-20 md:grid-cols-3">
          <div className="bg-panel p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Parsing runs in a dedicated Web Worker. Typed-array buffers are transferred (not
              copied) back to the UI, so a 100 MB session never blocks input or animation.
            </p>
          </div>
          <div className="bg-panel p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              uPlot is a non-React canvas renderer that handles hundreds of thousands of points per
              channel with a smooth, single-pixel cursor — exactly what MoTeC users expect.
            </p>
          </div>
          <div className="bg-panel p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Files are stored in a private bucket scoped by row-level security. Only your
              authenticated account can list, read or delete them.
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 py-20 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Create an account and drop in your first .ibt.
            </p>
          </div>
