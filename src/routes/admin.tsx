...
import { adminListUsers, adminSetRole, getMyRole, type ManagedUser, type UserRole } from "@/lib/admin.functions";
    ...
    if (myRole !== "admin") {
        return (
            <div className="flex min-h-screen flex-col items-center justify-center bg-background gap-4">
                ...
                <p className="text-sm text-muted-foreground">You need <span className="text-racing-red font-semibold">Admin</span> access to view this page.</p>
                <Link to="/" className="text-xs text-muted-foreground underline hover:text-foreground">← Back to home</Link>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-background text-foreground">
            <AppHeader>
                ...
                <Link to="/roadmap" className="ml-3 text-muted-foreground hover:text-foreground text-xs">Roadmap</Link>
            </AppHeader>
            ...
            <main className="mx-auto max-w-6xl px-6 py-8 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <StatCard label="Total Users" value={users.length} icon={<Users className="h-3.5 w-3.5" />} />
                    <StatCard label="Admins" value={adminCount} icon={<Shield className="h-3.5 w-3.5 text-racing-red" />} />
                    <StatCard label="Beta Testers" value={betaCount} icon={<Star className="h-3.5 w-3.5 text-racing-cyan" />} />
                    <StatCard label="Total Sessions" value={totalSessions} icon={<Database className="h-3.5 w-3.5 text-primary" />} />
                </div>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="text-sm">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                                Users promoted to <strong className="text-racing-cyan">Beta Tester</strong> get free access to all Pro features during the paid rollout period.
                                Once Stripe billing is live, their role grants them a permanent free subscription — ideal for community contributors, streamers, and sim coaches helping you shape the product.
                            </p>
                        </div>
                    </div>
                </div>
                                ...
                                <thead>
                                    <tr className="border-b border-border bg-rail text-xs font-mono uppercase tracking-wider text-muted-foreground">
                                        ...
                                        <th className="px-4 py-3 text-left hidden sm:table-cell">
                                            <div className="flex items-center gap-1"><Calendar className="h-3 w-3" /> Joined</div>
                                        </th>
                                        <th className="px-4 py-3 text-left hidden md:table-cell">
                                            <div className="flex items-center gap-1"><Activity className="h-3 w-3" /> Last Sign-in</div>
                                        </th>
                                        <th className="px-4 py-3 text-center hidden sm:table-cell">
                                            <div className="flex items-center justify-center gap-1"><Database className="h-3 w-3" /> Sessions</div>
                                        </th>
                                            ...
                                            return (
                                                <tr key={u.id} className="bg-panel hover:bg-rail transition-colors">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <div className="font-medium text-foreground truncate max-w-[200px]">{u.email}</div>
                                                        <div className="text-xs text-muted-foreground font-mono mt-0.5 hidden md:block truncate">{u.id}</div>
                                                    </td>
                                                    {/* Joined */}
                                                    ...
                                                    <td className="px-4 py-3 text-center hidden sm:table-cell">
                                                        <span className={`text-xs font-mono ${u.session_count > 0 ? "text-primary" : "text-muted-foreground/40"}`}>
                                                            {u.session_count}
                                                        </span>
                                                    </td>
                                                    {/* Role */}
