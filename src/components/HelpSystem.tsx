...
const steps: HelpStep[] = [
    ...
    content: (
      <div className="space-y-4">
        <p className="text-muted-foreground leading-relaxed">
          Pit Wall is a <strong className="text-foreground">three-in-one</strong> iRacing companion
          built for serious drivers. It combines a live in-session dashboard, a deep lap analysis
          workbench, and an AI-powered race engineer into one seamless tool.
        </p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          {[
            {
              icon: <Wifi className="h-4 w-4 text-racing-cyan" />,
              label: "Live Bridge",
              desc: "Real-time telemetry while you're on track",
            },
            {
              icon: <LineChart className="h-4 w-4 text-racing-green" />,
              label: "Lap Workbench",
              desc: "Deep analysis of saved .ibt / .pwlap files",
            },
            {
              icon: <Brain className="h-4 w-4 text-racing-orange" />,
              label: "AI Coach",
        ...
        <p className="text-xs text-muted-foreground border border-border/50 rounded-md px-3 py-2 bg-rail/50">
          ...
          <strong className="text-foreground">?</strong> button in the top-right corner.
        </p>
      </div>
    ),
  },
    ...
    content: (
      <div className="space-y-4">
        <p className="text-muted-foreground leading-relaxed">
          ...
          <strong className="text-foreground">Pit Wall Bridge</strong> is a small Node.js app that
          reads that memory and broadcasts it over WebSocket to your browser. You can launch it with
          one click!
        </p>
        <div className="space-y-2">
          {[
            {
              n: 1,
              icon: <Zap className="h-3.5 w-3.5 text-racing-orange" />,
            ...
            {
              n: 3,
              icon: <Gauge className="h-3.5 w-3.5 text-racing-green" />,
              title: "Stream Live",
            ...
            >
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-primary text-[10px] font-mono text-primary-foreground">
                {s.n}
              </span>
              <div>
    ...
    content: (
      <div className="space-y-4">
        <p className="text-muted-foreground leading-relaxed">
          ...
          <strong className="text-foreground">/live</strong> dashboard streams 60Hz telemetry from
          iRacing directly to your browser. No API keys, no cloud, no latency.
        </p>
        ...
        <p className="text-xs text-muted-foreground">
          <strong className="text-foreground">Recording:</strong> Hit the red dot button to record a
          session as a <code className="text-xs bg-rail px-1 rounded">.pwlap</code> file. When you
          save it, it opens instantly in the Workbench — no upload wait time.
        </p>
      </div>
    ),
    ctaLabel: "Open Live Dashboard",
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        <p className="text-muted-foreground leading-relaxed">
          Load any <code className="text-xs bg-rail px-1 rounded">.ibt</code> or{" "}
          <code className="text-xs bg-rail px-1 rounded">.pwlap</code> file into the{" "}
          <strong className="text-foreground">Workbench</strong> for a full MoTeC-style breakdown.
          No subscription required — it all runs locally in your browser.
        </p>
        <div className="space-y-2 text-xs">
          {[
            {
              icon: <LineChart className="h-3 w-3 text-primary" />,
              label: "Stacked Traces",
              desc: "Overlay throttle, brake, steer, speed and any other channel across laps",
            },
            {
              icon: <Zap className="h-3 w-3 text-racing-orange" />,
              label: "Sector Analysis",
              desc: "Identify exactly which corner is costing you the most time",
            },
            {
              icon: <Brain className="h-3 w-3 text-racing-cyan" />,
              label: "AI Coach Report",
              desc: "GPT-powered post-session coaching based on your telemetry profile",
            },
            {
              icon: <FolderOpen className="h-3 w-3 text-racing-green" />,
        ...
        <p className="text-xs text-muted-foreground">
          🔑 <strong className="text-foreground">Tip:</strong> Don't have an iRacing .ibt file? Use
          the Lab to upload one directly from disk without needing to log in.
        </p>
      </div>
    ),
    ctaLabel: "Open Lab",
    ...
    content: (
      <div className="space-y-4">
        <p className="text-muted-foreground leading-relaxed">
          ...
          <strong className="text-foreground">Session Library</strong>. Sign in with your account to
          sync data to the cloud, or — if you've installed MongoDB locally — everything stores on
          your own machine.
        </p>
        <div className="grid grid-cols-2 gap-2 text-xs">
          {[
        ...
        <div className="flex items-start gap-2 rounded-md border border-emerald-500/20 bg-emerald-500/5 px-3 py-2 text-xs text-emerald-400">
          ...
          <span>
            <strong>Local-First Active:</strong> Select the "Continue as Local Developer" option.
            Telemetry records write instantly to your local MongoDB, and file binaries are cached
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        <p className="text-muted-foreground leading-relaxed">
          The <strong className="text-foreground">AI Coach</strong> listens to each completed lap
          and gives you a radio call — just like a real race engineer. It decides the{" "}
          <em className="text-foreground">tone</em> based on your performance:{" "}
          <span className="text-racing-green font-semibold">PUSH</span>,{" "}
          <span className="text-racing-orange font-semibold">HOLD</span>, or{" "}
          <span className="text-red-400 font-semibold">WARN</span>.
        </p>
        <div className="space-y-2 text-xs">
          {[
            {
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          <p>
            ⚙️ <strong className="text-foreground">Settings → AI Provider</strong> to switch between
            cloud GPT-4o and a local LLM (Ollama, LM Studio, etc.).
          </p>
          <p>
            🔇 Auto-speak mode reads the call out loud via TTS — toggle it per-session on the live
            dashboard.
          </p>
        </div>
      </div>
    ...
    content: (
      <div className="space-y-4">
        ...
        <p className="text-xs text-center text-muted-foreground">
          Remember: hit the <strong className="text-foreground">?</strong> button in the top-right
          corner anytime to re-open this guide.
        </p>
      </div>
    ),
  },
      ...
      {open && (
        ...
        >
          ...
          <div className="relative w-full max-w-2xl rounded-xl border border-border bg-background shadow-2xl flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex items-center gap-1.5 px-6 pt-4 pb-1 shrink-0 overflow-x-auto">
              {steps.map((s, i) => (
                <button
                  key={s.id}
                  onClick={() => setStep(i)}
                  className={`flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-mono uppercase tracking-wider transition-all whitespace-nowrap ${
                    i === step
            ...
            {/* Main content */}
            <div className="flex-1 overflow-y-auto px-6 py-4">{current.content}</div>
