...
export function LiveStrategy({ t }: { t: Telemetry }) {
  ...
  return (
    <div className={`bg-zinc-925 ring-1 ${isAlert ? (isHigh ? "ring-racing-red/60" : "ring-racing-orange/60") : "ring-white/5"} rounded-lg p-4`}>
      ...
      {call && !loading && (
        <div className={`rounded-md p-3 ${isAlert ? (isHigh ? "bg-racing-red/15" : "bg-racing-orange/15") : "bg-zinc-900/50"}`}>
          <div className="flex items-center gap-2 mb-1.5">
            {isAlert && (
              <span className={`flex items-center gap-1 rounded-sm px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wider ${isHigh ? "text-racing-red" : "text-racing-orange"} bg-black/30`}>
          ...
          <div className={`text-lg font-semibold leading-tight ${isAlert ? (isHigh ? "text-racing-red" : "text-racing-orange") : "text-zinc-300"}`}>
            {call.headline}
          </div>
