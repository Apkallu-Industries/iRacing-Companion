    ...
    <div className="flex flex-col h-full bg-panel p-4 hairline rounded-sm">
      ...
      {histogram && (
        <div className="mt-4 grid grid-cols-4 gap-2 text-[10px] font-mono text-muted-foreground border-t border-border pt-2">
          <div>Mean: <span className="text-foreground">{histogram.stats.mean.toFixed(2)}</span></div>
          <div>Median: <span className="text-foreground">{histogram.stats.median.toFixed(2)}</span></div>
          <div>StdDev: <span className="text-foreground">{histogram.stats.stdDev.toFixed(2)}</span></div>
          <div>Count: <span className="text-foreground">{histogram.stats.count}</span></div>
        </div>
      )}
    </div>
  );
