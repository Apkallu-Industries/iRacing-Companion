...
export function generateDriverFingerprint(
  ...
  const isSteeringAggressive = steering.steeringVelocity > 1.5 || steering.microCorrectionFrequencyHz > 4.0;
  const isUnstable = consistency.overallConsistencyScore < 75;

  if (isUnstable) {
    driverClass = "UNSTABLE_AGGRESSIVE";
    focusAreas.push("Establish consistency in initial braking points to prevent micro-correction steering catches.");
    ...
    focusAreas.push("Trace exit throttle re-application slightly earlier while completing brake-release rotation.");
  } else if (throttle.liftAndCoastDurationSec > 1.2 && !isThrottleAggressive) {
    driverClass = "SMOOTH_COAST_OPTIMIZER";
    focusAreas.push("Explore deeper entry lines to maximize mid-corner aerodynamic downforce flow.");
  } else if (brake.peakBrakePressure < 0.65 && steering.steeringSmoothnessIndex > 90) {
    driverClass = "CONSERVATIVE_FLUID";
    focusAreas.push("Increase peak braking threshold targets to compress deceleration zone distances.");
    focusAreas.push("Commit to earlier exit power sweeps.");
  } else {
    driverClass = "BALANCED_COMPOSITE";
    focusAreas.push("Optimize ERS state-of-charge decay through selective straightaway lift-and-coast.");
  }

  if (brake.lockupTendencyIndex > 0.15) {
    focusAreas.push("Reduce trail-braking foot pressure under high steering rotation angles to prevent front-wheel lockups.");
  }
  if (steering.microCorrectionFrequencyHz > 3.5) {
    focusAreas.push("Calibrate shock rebound dampers or soften roll-bars to settle chassis understeer catches.");
  }
