...
export function calculateSessionCorrelations(
  ...
  const matchingChanges = historicalChanges.filter(
    (c) =>
      (c.change_type.toLowerCase() === "rear rebound" && c.parameter.includes(">")) ||
      c.notes.toLowerCase().includes("rebound"),
  );
  ...
  if (isSpaT8Trap) {
    return {
      ...
      recommendedPreemptiveDelta:
        "Pre-emptively soften rear rebound dampers by -1 click or lift exit packers by +1.0mm to recover vacuum seal flow.",
    };
  }
  ...
  if (matchingEvents.length > 3 && matchingChanges.length > 0) {
    return {
      ...
      recommendedPreemptiveDelta:
        "Align damper compression velocities. Increase high-speed front packers by +1.5mm to restrict heave rates.",
    };
  }
  ...
  return {
    ...
    narrativeDescription:
      "Chassis inputs and setup profiles indicate low-risk telemetry envelopes. Normal vehicle platform stability parameters active.",
