...
3. **Install Dependencies**:
   ...
   npm install
   ```
   *This automatically resolves, downloads, and compiles dependencies for the client, desktop shell, and bridge simultaneously.*
...
1. **Start the Web Client (Terminal 1)**:
   ...
   npm run dev
   ```
   *The client dashboard is now live at `http://localhost:3000` (or `http://localhost:8080`).*
...
2. **Start the Live Bridge (Terminal 2)**:
   ...
   npm start
   ```
   *The bridge connects to iRacing, registers MongoDB recording, and establishes a 60Hz telemetry socket on port `3001`.*
