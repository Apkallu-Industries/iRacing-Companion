...
function triggerLockupLF(
  ...
  if (profile.className === "Rallycross") {
    ...
    if (!checkAndSetCooldown(RaceEventType.BRAKE_LOCK_FRONT_LEFT)) {
      events.push(
        createTelemetryEvent({
          ...
          sourceTypes: ["deterministic_physics"],
          rationale: [
            "wheel_speed_decayed_below_chassis_speed",
            "brake_pressure_applied",
            "abs_modulation_active",
          ],
          classification: "observed",
          latency,
          profile,
    ...
    if (!checkAndSetCooldown(RaceEventType.BRAKE_LOCK_FRONT_LEFT)) {
      events.push(
        createTelemetryEvent({
          ...
          rationale: [
            "wheel_speed_decayed_below_chassis_speed",
            "raw_brake_pedal_exceeded_grip_limit",
            "absence_of_abs",
          ],
          classification: "observed",
          latency,
    ...
    profile.className === "LMP2" ||
    profile.className === "GTP" ||
    profile.className === "SF23"
  ) {
    if (!checkAndSetCooldown(RaceEventType.BRAKE_LOCK_FRONT_LEFT)) {
      events.push(
        createTelemetryEvent({
          ...
          rationale: [
            "wheel_speed_decayed_below_chassis_speed",
            "downforce_load_decay_under_deceleration",
            "absence_of_abs",
          ],
          classification: "observed",
          latency,
...
function evaluateEvents(current, ringBuffer, rawOriginal = {}, vehicleProfile = null) {
  ...
  if (!current.estimation) {
    ...
    const prevFrame =
      ringBuffer && ringBuffer.buffer && ringBuffer.buffer.length > 0
  ...
  const rearHeight =
    rawOriginal.RrideHeight ?? (rawOriginal.rearRideHeight ? rawOriginal.rearRideHeight / 1000 : 0);
  ...
  if (capabilities.brakeLockDetect !== "None") {
    ...
    if (C_lock_fl >= 0.4 && lfSpeed !== undefined) {
      triggerLockupLF(
        lfSpeed,
        speed,
        current,
    ...
    if (C_lock_fr >= 0.4 && rfSpeed !== undefined) {
      triggerLockupRF(
        rfSpeed,
        speed,
        current,
    ...
    if (!checkAndSetCooldown(RaceEventType.REAR_TRACTION_COLLAPSE)) {
      events.push(
        createTelemetryEvent({
          ...
          sourceTypes: ["deterministic_physics"],
          rationale: [
            "rear_driven_wheels_exceeded_chassis_velocity",
            "throttle_input_applied",
            "tc_saturation",
          ],
          classification: "observed",
          latency,
  ...
  if (capabilities.aeroStallDetect !== "None" && C_bottom_out >= 0.4) {
    ...
    if (!checkAndSetCooldown(RaceEventType.AERO_BOTTOM_OUT)) {
      events.push(
        createTelemetryEvent({
          ...
          sourceTypes: ["deterministic_physics"],
          rationale: [
            "vertical_damper_velocity_exceeded_threshold",
            "high_speed_aerodynamic_load_compression",
          ],
          classification: "observed",
          latency,
          profile,
  ...
  if (C_over_rotation >= 0.4 && derived.oversteerIndex) {
    ...
    if (!checkAndSetCooldown(RaceEventType.ENTRY_OVER_ROTATION)) {
      events.push(
        createTelemetryEvent({
          ...
          rationale: [
            "actual_yaw_rate_exceeded_kinematic_expectation",
            "abrupt_brake_release_gradient",
            "rear_tire_slip_angle_spike",
          ],
  ...
  if (C_understeer >= 0.4 && derived.understeerIndex) {
    ...
    if (!checkAndSetCooldown(RaceEventType.EXIT_UNDERSTEER)) {
      events.push(
        createTelemetryEvent({
          ...
          sourceTypes: ["deterministic_physics"],
          rationale: [
            "actual_yaw_rate_falls_short_of_kinematic_path",
            "throttle_exit_acceleration_slip",
          ],
          classification: "interpreted",
          latency,
          profile,
      ...
      if (!checkAndSetCooldown(RaceEventType.AERO_PLATFORM_OSCILLATION)) {
        events.push(
          createTelemetryEvent({
            ...
            sourceTypes: ["deterministic_physics"],
            rationale: [
              "high_speed_aerodynamic_pitch_resonance",
              "out_of_phase_damper_deflection",
              "aero_heave_packers_saturated",
            ],
            classification: "observed",
            latency,
            profile,
      ...
      if (!checkAndSetCooldown(RaceEventType.DIFFUSER_STALL)) {
        events.push(
          createTelemetryEvent({
            ...
            sourceTypes: ["deterministic_physics"],
            rationale: [
              "rear_ride_height_below_stall_limit",
              "venturi_pressure_seal_ruptured",
              "underbody_aerodynamic_choking",
            ],
            classification: "observed",
            latency,
            profile,
  ...
  if (profile.architecture?.abs === false && C_brake_migration >= 0.4 && derived.oversteerIndex) {
    if (!checkAndSetCooldown(RaceEventType.BRAKE_MIGRATION_ROTATION)) {
      events.push(
        createTelemetryEvent({
          ...
          sourceTypes: ["deterministic_physics", "behavioral_model"],
          rationale: [
            "heavy_trail_braking_deceleration",
            "brake_bias_migration_limits_exceeded",
            "front_axle_contact_patch_saturation",
          ],
          classification: "interpreted",
          latency,
          profile,
  ...
  if (profile.architecture?.hybrid === true && C_hybrid >= 0.4) {
    const lrSpeed = rawOriginal.LRwheelSpeed ?? rawOriginal.lrSpeed;
    const rrSpeed = rawOriginal.RRwheelSpeed ?? rawOriginal.rrSpeed;
    ...
    if (!checkAndSetCooldown(RaceEventType.HYBRID_DEPLOYMENT_SURGE)) {
      events.push(
        createTelemetryEvent({
          ...
          sourceTypes: ["deterministic_physics"],
          rationale: [
            "hybrid_torque_fill_onset",
            "driven_wheel_speed_differential_asymmetry",
            "exit_traction_limit_exceeded",
          ],
          classification: "observed",
          latency,
          profile,
    ...
    if (!checkAndSetCooldown(RaceEventType.FLOOR_DAMAGE_ALERT)) {
      events.push(
        createTelemetryEvent({
          ...
          rationale: [
            "bayesian_latent_fault_inference",
            "sustained_underbody_impacts",
            "coupled_platform_resonance",
          ],
          classification: "interpreted",
          latency,
          profile,
    ...
    if (!checkAndSetCooldown(RaceEventType.TIRE_DEGRADATION_ALERT)) {
      events.push(
        createTelemetryEvent({
          ...
          rationale: [
            "bayesian_latent_fault_inference",
            "cumulative_axle_slip_energy",
            "lateral_adhesion_decay",
          ],
          classification: "interpreted",
          latency,
          profile,
...
function createTelemetryEvent(opts) {
  ...
  const baseConfidence = opts.confidence ?? 0.9;
  ...
  let weightedConfidence =
    typeof eventPersistedConf === "number"
      ? eventPersistedConf
      : getSourceWeightedConfidence(baseConfidence, sourceTypes);

  // High-order Bayesian probability mapping backstop
  if (
    opts.eventType === RaceEventType.FLOOR_DAMAGE_ALERT &&
    current?.estimation?.hypotheses?.floorDamage
  ) {
    weightedConfidence = current.estimation.hypotheses.floorDamage.value;
  }
  if (
    opts.eventType === RaceEventType.TIRE_DEGRADATION_ALERT &&
    current?.estimation?.hypotheses?.tireDegradation
  ) {
  ...
  const isAeroEvent =
    opts.eventType === RaceEventType.AERO_BOTTOM_OUT ||
  ...
  return {
    ...
    physicsProfileVersion:
      opts.profile?.physicsProfileVersion || opts.physicsProfileVersion || "1.0.0",
  };
}
