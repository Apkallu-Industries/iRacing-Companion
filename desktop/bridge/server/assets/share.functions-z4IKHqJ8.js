...
const createShareLink = createServerFn({
  ...
  .handler(createShareLink_createServerFn_handler, async ({ data, context }) => {
    ...
    const expiresAt =
      data.expiresInDays && data.expiresInDays > 0
        ? new Date(Date.now() + data.expiresInDays * 864e5).toISOString()
        : null;
...
const revokeShareLink = createServerFn({
  ...
  .handler(revokeShareLink_createServerFn_handler, async ({ data, context }) => {
...
const refreshSharedSignedUrl = createServerFn({
  ...
  .handler(refreshSharedSignedUrl_createServerFn_handler, async ({ data }) => {
...
const getSharedLap = createServerFn({
  ...
  .handler(getSharedLap_createServerFn_handler, async ({ data }) => {
