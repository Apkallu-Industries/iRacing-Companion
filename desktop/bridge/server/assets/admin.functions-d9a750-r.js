...
const adminListUsers = createServerFn({
  ...
  .handler(adminListUsers_createServerFn_handler, async ({ context }) => {
    const { supabase, userId } = context;
    await requireAdmin(supabase, userId);
    const { data: roles } = await supabase.from("user_roles").select("user_id, role");
    const { data: sessions } = await supabase.from("telemetry_sessions").select("user_id");
    let authUsers = [];
    try {
...
const adminSetRole = createServerFn({
  ...
  .handler(adminSetRole_createServerFn_handler, async ({ data, context }) => {
    const { supabase, userId } = context;
    await requireAdmin(supabase, userId);
...
const getMyRole = createServerFn({
  ...
  .middleware([requireSupabaseAuth])
  .handler(getMyRole_createServerFn_handler, async ({ context }) => {
    const { supabase, userId } = context;
