...
import {
  c as connectToLocalDb,
  r as readDbConfig,
  w as writeDbConfig,
  a as resetDbConnection,
} from "./db.local-BHO1WuOq.js";
...
async function createObjectId(id) {
  ...
  const { ObjectId } = await dynamicImport("mongodb");
...
const getLocalSessionById = createServerFn({
  ...
  .handler(getLocalSessionById_createServerFn_handler, async ({ data: id }) => {
    try {
      const db = await connectToLocalDb();
...
const insertLocalSession = createServerFn({
  ...
  .handler(insertLocalSession_createServerFn_handler, async ({ data }) => {
    try {
      const db = await connectToLocalDb();
      const doc = {
        ...data,
...
const deleteLocalSession = createServerFn({
  ...
  .inputValidator((id) => String(id))
  .handler(deleteLocalSession_createServerFn_handler, async ({ data: id }) => {
    try {
      const db = await connectToLocalDb();
      const objectId = await createObjectId(id);
...
const saveDbConfig = createServerFn({
  ...
  .handler(saveDbConfig_createServerFn_handler, async ({ data: config }) => {
    try {
