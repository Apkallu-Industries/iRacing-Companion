...
const upsertFingerprint = createServerFn({
  ...
  return data;
}).handler(upsertFingerprint_createServerFn_handler, async ({
  data,
  context
}) => {
  const {
  ...
  const rows = data.pairs.filter((p) => p.track && p.car && isFinite(p.bestEverS) && p.bestEverS > 0).map((p) => ({
    user_id: userId,
    track: p.track,
    car: p.car,
  ...
  const {
    error
  } = await supabase.from("driver_fingerprint").upsert(rows, {
...
const getFingerprintForPair = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((data) => data).handler(getFingerprintForPair_createServerFn_handler, async ({
  data,
  context
}) => {
  const {
  ...
  const {
    data: exact
  } = await supabase.from("driver_fingerprint").select("*").ilike("track", data.track).ilike("car", data.car).limit(1).maybeSingle();
  if (exact) return {
    fp: exact
  };
  const {
    data: rows
  } = await supabase.from("driver_fingerprint").select("*").ilike("track", `%${data.track}%`).limit(20);
  const match = (rows ?? []).find((r) => r.track.toLowerCase().includes(t) && r.car.toLowerCase().includes(c));
  return {
    fp: match ?? null
  };
});
...
const getLastSessionForPair = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((data) => data).handler(getLastSessionForPair_createServerFn_handler, async ({
  data,
  context
}) => {
  const {
  ...
  const {
    data: rows
  } = await supabase.from("telemetry_sessions").select("id,name,recorded_at,best_lap_s,track,car,storage_path,fingerprint_delta").ilike("track", `%${data.track}%`).ilike("car", `%${data.car}%`).order("recorded_at", {
    ascending: false,
    nullsFirst: false
  }).limit(1);
  return {
...
const hasAnyFingerprint = createServerFn({
  method: "GET"
}).middleware([requireSupabaseAuth]).handler(hasAnyFingerprint_createServerFn_handler, async ({
  context
}) => {
  const {
    supabase
  ...
  const {
    count
  } = await supabase.from("driver_fingerprint").select("user_id", {
    count: "exact",
    head: true
  });
  return {
...
const updateSessionFingerprintDelta = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((data) => data).handler(updateSessionFingerprintDelta_createServerFn_handler, async ({
  data,
  context
}) => {
  const {
