...
import { G as Ae, Z as ft, b as Ce } from "./index-BF1LFLDu.js";
import { i as Ye, n as ie, d as fe } from "./vendor-CUluG-o1.js";
import { ap as bt, g as ut, aG as jt, aU as be } from "./icons-UNkcvPbk.js";
...
function gt(a) {
    ...
    A = r.useCallback(() => {
      U((x) => {
        const f = Date.now(),
          m = new Map(x);
        for (const [g, B] of m) {
          const v = f - B.timestamp;
          m.set(g, { ...B, staleness: v, isOnline: v < Nt });
        }
        return m;
      });
    }, []);
  r.useEffect(() => {
    ...
    const x = `team:${a}`;
    w.current && Ae.removeChannel(w.current);
    const f = Ae.channel(x, { config: { broadcast: { ack: !1 } } });
    return (
      f.on("broadcast", { event: "telemetry" }, ({ payload: m }) => {
        if (!m?.carNumber) return;
        const g = m.carOperationalState,
          B = g?.activeDriver || "Unknown Driver",
          v = g?.fatigueSummary?.brakes ?? 100,
          N = {
            carNumber: m.carNumber,
            carName: m.carName || "Unknown Car",
            driverName: B,
            carOperationalState: g,
            lastLapSec: 0,
            lastLap: "--:--.---",
            bestLap: "--:--.---",
            deltaSec: 0,
            fuelRemainingL: 0,
            fuelBurnPerLap: 0,
            lapsEstimated: g?.projectedPitLap ? Math.max(0, g.projectedPitLap) : 0,
            speedKph: 0,
            gear: 0,
            rpm: 0,
            trackTempC: 0,
            trackWetness: 0,
            tires: g?.fatigueSummary
              ? {
                  fl: {
                    tempC: 80,
                    pressureBar: 2,
              ...
              : null,
            enduranceState: g?.fatigueSummary
              ? {
                  chassisFatigue: g.fatigueSummary.chassis,
                  brakeWear: g.fatigueSummary.brakes,
                  gearboxStress: g.fatigueSummary.gearbox,
                  ersHealth: g.fatigueSummary.ersHealth,
                }
              : null,
            adaptationState: g?.adaptationWindow
              ? {
                  event: g.adaptationWindow.active
                    ? "DRIVER_ADAPTATION_ACTIVE"
                    : "DRIVER_ADAPTATION_INACTIVE",
                  incomingDriver: B,
                  currentLapInWindow: g.adaptationWindow.currentLapInWindow,
            ...
            publishCount: m.publishCount || 1,
            isOnline: !0,
            staleness: 0,
          };
        U((q) => {
          const P = q.get(m.carNumber);
          if (P) {
            const X = P.carOperationalState?.sequenceId ?? 0,
              L = N.carOperationalState?.sequenceId ?? 0,
              C = P.timestamp ?? 0,
              E = N.timestamp ?? 0;
            if (L < X || (L === X && E <= C))
              return (
                console.warn(
                  `[telemetry-sync] Discarded out-of-order/stale packet: Car ${m.carNumber} | Inbound Seq: ${L} (Existing: ${X}) | Inbound Ts: ${E} (Existing: ${C})`,
                ),
                q
              );
          }
          const D = new Map(q);
          return (D.set(m.carNumber, N), D);
        });
      }),
      f.subscribe((m) => {
        z(m === "SUBSCRIBED");
      ...
      (w.current = f),
      (k.current = setInterval(A, 5e3)),
      () => {
        (w.current && (Ae.removeChannel(w.current), (w.current = null)),
          k.current && (clearInterval(k.current), (k.current = null)),
          z(!1));
      }
    );
  }, [a, A]);
  const K = Array.from(O.values());
  return {
    drivers: O,
    connected: ue,
    teamCode: a,
    driverCount: O.size,
    onlineCount: K.filter((x) => x.isOnline).length,
  };
}
function vt(a) {
  const O = Math.floor(a / 60),
    U = Math.round(a % 60);
  return O > 0 ? `${O}h ${U}m` : `${U}m`;
}
function de({ data: a, color: O }) {
  const z = Math.max(...a, 1),
    w = Math.min(...a, 0),
    k = z - w || 1,
    A = a
      .map((K, x) => {
        const f = (x / (a.length - 1)) * 140,
          m = 14 - ((K - w) / k) * 14;
        return `${f},${m}`;
      })
      .join(" ");
  return e.jsx("svg", {
    ...
    children: e.jsx("polyline", { fill: "none", stroke: O, strokeWidth: "1.1", points: A }),
  });
}
function Vt() {
  const a = ft(),
    ...
    [A, K] = r.useState(null),
    [x, f] = r.useState(null),
    [m, g] = r.useState("green"),
    [B, v] = r.useState("6h"),
    [N, q] = r.useState(() => {
      const t = typeof window < "u" ? localStorage.getItem("team_race_duration_h") : null;
      return t ? Number(t) : 6;
    });
  r.useEffect(() => {
    localStorage.setItem("team_race_duration_h", String(N));
  }, [N]);
  const [P, D] = r.useState(1),
    [X, L] = r.useState(0),
    C = N * 60,
    E = C / P,
    H = Math.max(0, Math.min(C - E, X)),
    Re = H + E,
    h = (t) => (((t / 600) * C - H) / E) * 600,
    R = (t) =>
      t.replace(
        ...
        (s, n, l, c, o, d, i, j) => {
          const T = h(parseFloat(l));
          if (o !== void 0 && d !== void 0) {
            const Y = h(parseFloat(o));
            if (i !== void 0 && j !== void 0) {
              const ce = h(parseFloat(i));
              return `${n} ${T},${c} ${Y},${d} ${ce},${j}`;
            }
            return `${n} ${T},${c} ${Y},${d}`;
          }
          return `${n} ${T},${c}`;
    ...
    b = (t) => {
      const s = C / 4,
        n = t * s,
        l = (t + 1) * s;
      if (!(l > H && n < Re)) return { display: "none" };
      const o = Math.max(0, ((n - H) / E) * 100),
        d = Math.max(0, ((Re - l) / E) * 100),
        i = 100 - o - d;
      return {
        position: "absolute",
        left: `calc(${o}% + 1px)`,
    ...
    Me = (t) => {
      ...
      const s = t.currentTarget.getBoundingClientRect(),
        l = t.currentTarget.classList.contains("timeline-graph-container") ? 0 : 110,
        c = t.clientX - s.left - l,
        o = s.width - l;
      if (o <= 0) return;
      const d = Math.max(0, Math.min(1, c / o)),
        i = H + d * E,
        j = t.deltaY < 0 ? 1.25 : 1 / 1.25,
        T = Math.min(48, Math.max(1, P * j));
      if (T === P) return;
      const Y = C / T;
      let ce = i - d * Y;
      ((ce = Math.max(0, Math.min(C - Y, ce))), D(T), L(ce));
    },
    Oe = (t) => {
      t.buttons === 3 &&
        (t.preventDefault(), (ee.current = { startX: t.clientX, startPanOffset: X }));
    },
    Be = (t) => {
      if (ee.current && t.buttons === 3) {
        t.preventDefault();
        const s = t.currentTarget.getBoundingClientRect(),
          l = t.currentTarget.classList.contains("timeline-graph-container") ? 0 : 110,
          c = s.width - l;
        if (c <= 0) return;
        const d = ((t.clientX - ee.current.startX) / c) * E;
        let i = ee.current.startPanOffset - d;
        ((i = Math.max(0, Math.min(C - E, i))), L(i));
      }
    },
    xe = () => {
      ee.current = null;
    ...
    [S, ye] = r.useState(() =>
      typeof window < "u" ? (localStorage.getItem("team_code") ?? "") : "",
    ),
    [Fe, ke] = r.useState(""),
    [qe, te] = r.useState(!1);
  r.useEffect(() => {
    typeof window < "u" && localStorage.setItem("team_code", S);
  }, [S]);
  const { drivers: Z, connected: J } = gt(S || null),
    et = () => {
      const s = `PITWALL-${Math.random().toString(36).slice(2, 6).toUpperCase()}`;
      (ye(s), ke(s));
    },
    [y, se] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_drivers");
    ...
    [I, Ee] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_cars");
        if (t) return JSON.parse(t);
      }
      return [
    ...
    [M, Se] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_stints");
        if (t) return JSON.parse(t);
      }
      return [
        {
    ...
    [_e, wt] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_weather");
    ...
    [We, yt] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_incidents");
        return t ? JSON.parse(t) : [];
      }
      return [];
    }),
    [V, Ft] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_race_start_time");
        return t || "2026-05-28T15:00:00.000Z";
      }
      return "2026-05-28T15:00:00.000Z";
    }),
    [u, ne] = r.useState(() => {
      if (typeof window < "u") {
        const t = localStorage.getItem("team_selected_car_id");
        return t || "c1";
      }
      return "c1";
    }),
    [$e, tt] = r.useState(new Date()),
    [_, Te] = r.useState(2.85),
    [W, st] = r.useState(45),
    [re, Ue] = r.useState(95);
  (r.useEffect(() => {
    localStorage.setItem("team_drivers", JSON.stringify(y));
  }, [y]),
    r.useEffect(() => {
      (localStorage.setItem("team_cars", JSON.stringify(I)),
        I.length > 0 ? (!u || !I.some((t) => t.id === u)) && ne(I[0].id) : ne(null));
    }, [I, u]),
    r.useEffect(() => {
      localStorage.setItem("team_stints", JSON.stringify(M));
    }, [M]),
    r.useEffect(() => {
      localStorage.setItem("team_weather", JSON.stringify(_e));
    }, [_e]),
    r.useEffect(() => {
      localStorage.setItem("team_incidents", JSON.stringify(We));
    }, [We]),
    r.useEffect(() => {
      localStorage.setItem("team_race_start_time", V);
    }, [V]),
    r.useEffect(() => {
      u
        ? localStorage.setItem("team_selected_car_id", u)
        : localStorage.removeItem("team_selected_car_id");
    }, [u]));
  const [nt, De] = r.useState(!1),
    [rt, Le] = r.useState(!1),
    [kt, Et] = r.useState(!1),
    [St, Tt] = r.useState(!1),
    [Dt, Lt] = r.useState(!1),
    [G, he] = r.useState({ name: "", number: "", carClass: "GT3" }),
    [$, me] = r.useState({ name: "", shortName: "", color: "#3B82F6", status: "Available" }),
    [It, At] = r.useState({ type: "Sunny", startOffset: 0, duration: 60 }),
    [Ct, Rt] = r.useState({ carId: "", driverId: "", startOffset: 0, duration: 60, note: "" }),
    [Mt, Ot] = r.useState({ type: "Caution", startOffset: 0, duration: 15 }),
    [Ge, at] = r.useState({ isRunning: !1, elapsed: 0, lastStart: null });
  (r.useEffect(() => {
    const t = setInterval(() => {
      tt(new Date());
    }, 1e3);
    ...
    r.useEffect(() => {
      if (!Ge.isRunning) return;
      const t = setInterval(() => {
        at((s) => {
          const n = Date.now(),
            l = s.lastStart ?? n;
          return { ...s, elapsed: s.elapsed + (n - l) / 1e3, lastStart: n };
        });
      }, 100);
      return () => clearInterval(t);
    }, [Ge.isRunning]));
  const F = I.find((t) => t.id === u) || I[0],
    p = r.useMemo(() => (F && Z.get(F.number)) || null, [Z, F]),
    He = r.useRef(null);
  (r.useEffect(() => {
    He.current = p;
  }, [p]),
    r.useEffect(() => {
      if (!S || Z.size === 0) return;
      let t = !1,
        s = !1;
      const n = [...I],
        l = [...y];
      for (const [c, o] of Z.entries())
        if (
          (!n.find((i) => i.number === c) &&
            o.carName &&
            (n.push({
              ...
              name: o.carName,
              number: c,
              carClass: o.carName.toUpperCase().includes("GT3")
                ? "GT3"
                : o.carName.toUpperCase().includes("LMP2")
                  ? "LMP2"
                  : "GTP",
            }),
            (t = !0)),
          o.driverName && o.driverName !== "Unknown Driver")
        ) {
          const i = l.find((j) => j.name.toLowerCase() === o.driverName.toLowerCase());
          if (i) {
            const j = o.isOnline ? "Driving" : "Available";
            i.status !== j && ((i.status = j), (s = !0));
          } else {
            const T = (o.driverName.split(" ").slice(-1)[0] || "DRV").slice(0, 3).toUpperCase();
            (l.push({
              id: `d_discovered_${o.carNumber}_${Date.now()}`,
              name: o.driverName,
              shortName: T,
              color:
                "#" +
                Math.floor(Math.random() * 16777215)
                  .toString(16)
                  .padStart(6, "0"),
              status: o.isOnline ? "Driving" : "Available",
            }),
              (s = !0));
          }
        }
  ...
  const ot = r.useMemo(() => Number((_ * W).toFixed(2)), [_, W]);
  (r.useMemo(() => Number((_ * 1.5).toFixed(2)), [_]),
    r.useMemo(() => {
      const t = F?.carClass === "GTP" ? 1.65 : F?.carClass === "LMP2" ? 1.45 : 1.25;
      return Number(t.toFixed(2));
    }, [F]));
  const lt = r.useMemo(() => W * (re / 60), [W, re]),
    Q = r.useMemo(() => Ye($e, ie(V)) * 60 * 1e3, [$e, V]),
    ae = N * 60 * 60 * 1e3;
  (r.useMemo(() => Math.max(0, ae - Math.max(0, Q)), [ae, Q]),
    r.useMemo(() => (ae <= 0 ? 0 : Math.min(100, (Math.max(0, Q) / ae) * 100)), [Q, ae]),
    r.useMemo(() => {
      const t = ie(V).getHours(),
        s = Math.max(0, Q) / 36e5,
        n = (t + s) % 24;
      return n >= 6 && n < 9
        ? {
    ...
    r.useMemo(() => {
      const t = {};
      for (const s of M) {
        const n = ie(s.startTime),
          l = ie(s.endTime),
          c = Ye(l, n) / 60;
        t[s.driverId] = (t[s.driverId] || 0) + c;
      }
      return t;
    }, [M]));
  const Ie = r.useMemo(() => {
      if (_ <= 0 || W <= 0) return null;
      const t = N * 3600,
        s = Math.round(t / re),
        n = _ * W,
        l = Math.ceil(s / W) - 1,
        c = _ * s;
      return { totalLapsEst: s, pitStops: l, fuelPerStint: n, totalFuel: c };
    }, [N, _, W, re]),
    oe = ie(V),
    ct = (t) => {
      if (
        (Ee((s) => s.filter((n) => n.id !== t)), Se((s) => s.filter((n) => n.carId !== t)), u === t)
      ) {
        const s = I.filter((n) => n.id !== t);
        ne(s.length > 0 ? s[0].id : null);
      }
    },
    it = (t) => {
      (se((s) => s.filter((n) => n.id !== t)), Se((s) => s.filter((n) => n.driverId !== t)));
    },
    dt = (t) => {
      if ((t.preventDefault(), !G.name || !G.number)) return;
      const s = { id: `c_${Date.now()}`, name: G.name, number: G.number, carClass: G.carClass };
      (Ee((n) => [...n, s]), he({ name: "", number: "", carClass: "GT3" }), De(!1), u || ne(s.id));
    },
    xt = (t) => {
      if ((t.preventDefault(), !$.name || !$.shortName)) return;
      const s = {
        id: `d_${Date.now()}`,
        name: $.name,
      ...
      (se((n) => [...n, s]),
        me({ name: "", shortName: "", color: "#3B82F6", status: "Available" }),
        Le(!1));
    },
    Ve = (t, s) => {
      if (!u) return;
      const n = `stint_${u}_${t}`,
        l = M.findIndex((o) => o.carId === u && o.id === n),
        c = [...M];
      if (l !== -1) c[l] = { ...c[l], driverId: s };
      else {
        const o = fe(oe, t * 90).toISOString(),
          d = fe(oe, (t + 1) * 90).toISOString();
        c.push({
          id: n,
          carId: u,
          driverId: s,
    ...
    pt = r.useMemo(() => {
      ...
      const t = [];
      for (let s = 0; s < 4; s++) {
        const n = `stint_${u}_${s}`,
          l = M.find((c) => c.carId === u && c.id === n);
        if (l) t.push(l);
        else {
          const c = fe(oe, s * 90).toISOString(),
            o = fe(oe, (s + 1) * 90).toISOString();
          t.push({
            id: n,
            carId: u,
            driverId: "",
    ...
    [le, ht] = r.useState({
      ...
      rpm: Array.from({ length: 25 }, () => 5e3),
      fuel: Array.from({ length: 25 }, () => 54),
    });
  r.useEffect(() => {
    const t = setInterval(() => {
      ht((s) => {
        const n = (T, Y) => [...T.slice(1), Y],
          l = He.current,
          c =
            l && l.speedKph > 0
              ? l.speedKph
          ...
          o = a.connected ? a.throttle : Math.round(40 + Math.random() * 60),
          d = a.connected ? a.brake : Math.round(Math.random() * 10),
          i = l && l.rpm > 0 ? l.rpm : a.connected ? a.rpm : Math.round(5500 + Math.random() * 2e3),
          j =
            l && l.fuelRemainingL > 0
              ? l.fuelRemainingL
              : a.connected
                ? a.fuelRemainingL
    ...
    return () => clearInterval(t);
  }, [a.connected, a.speedKph, a.throttle, a.brake, a.rpm, a.fuelRemainingL]);
  const mt = (t) => {
    se((s) =>
      s.map((n) => {
        if (n.id !== t) return n;
        ...
        children: [
          e.jsxs("div", {
            ...
            children: [
              ...
              e.jsx("span", { className: "h-3.5 w-px bg-[#1c2430]" }),
              e.jsx("span", {
                className:
              ...
              e.jsx("span", { className: "h-3.5 w-px bg-[#1c2430] hidden sm:inline" }),
              e.jsx(Ce, {
                to: "/team-guide",
                className:
          ...
          e.jsxs("div", {
            ...
            children: [
              ...
              e.jsx("div", { className: "h-3 w-px bg-[#1c2430]" }),
              e.jsxs("div", {
          ...
          e.jsxs("div", {
            ...
            children: [
              S &&
                e.jsx("span", {
                  onClick: () => te(!0),
                  className: `text-[8px] font-black uppercase tracking-widest border px-1.5 py-0.5 rounded-none cursor-pointer transition-all ${J ? "text-[#00D17F] border-[#00D17F]/25 bg-[#00D17F]/5 hover:bg-[#00D17F]/10" : "text-[#FF4D4D] border-red-500/25 bg-red-500/5 hover:bg-red-500/10 animate-pulse"}`,
                  children: J ? `● SECURE RELAY: ${S}` : "○ OFFLINE RELAY",
                }),
              e.jsx("button", {
                type: "button",
                onClick: () => Qe(!we),
                className: `text-[8px] font-black uppercase tracking-widest border px-1.5 py-0.5 rounded-none cursor-pointer transition-all ${we ? "text-[#FFB800] border-[#FFB800]/30 bg-[#FFB800]/10 animate-pulse" : "text-[#7a828c] hover:text-white border-[#1c2430] bg-[#0b0f14]"}`,
                children: "🔧 SETTINGS",
              }),
              e.jsx(Ce, {
                to: "/settings",
                ...
                children: e.jsx(bt, { className: "w-3.5 h-3.5" }),
              }),
            ],
          }),
        ],
              ...
              e.jsxs("div", {
                ...
                children: [
                  ...
                  e.jsx("div", {
                    ...
                    children: I.map((t) => {
                      const s = t.id === u,
                        n = M.filter((c) => c.carId === t.id),
                        l = Array.from(
                          new Set(
                            n
                              .map((c) => {
                                const o = y.find((d) => d.id === c.driverId);
                                return o ? o.name.split(" ").slice(-1)[0] : null;
                              })
                              .filter(Boolean),
                          ),
                        );
                      return e.jsxs(
                        ...
                        {
                          onClick: () => ne(t.id),
                          className: `p-2 rounded-none border transition-all text-left relative cursor-pointer group flex items-start gap-2.5 ${s ? "bg-[#3B82F6]/5 border-[#3B82F6]/55 shadow-[0_0_8px_rgba(59,130,246,0.1)]" : "bg-[#05070a]/60 border-[#1c2430] hover:border-[#7a828c]/40 hover:bg-[#11161d]"}`,
                          children: [
                            e.jsx("div", {
                              className: `p-1.5 rounded-none bg-[#05070a] border border-[#1c2430] flex items-center justify-center shrink-0 ${s ? "text-[#3B82F6]" : "text-[#7a828c]"}`,
                              children: e.jsx(ut, { className: "w-4 h-4" }),
                            }),
                            e.jsxs("div", {
                            ...
                            e.jsx("button", {
                              ...
                              title: "Evict car",
                              children: e.jsx(jt, { className: "w-2.5 h-2.5" }),
                            }),
                          ],
                        },
                        t.id,
              ...
              e.jsxs("div", {
                ...
                children: [
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("span", {
                        ...
                        children: [
                          "2 DRIVERS (",
                          F ? F.name.toUpperCase() : "NO CAR SELECTED",
                          ")",
                        ],
                      }),
                      e.jsx("button", {
                  ...
                  e.jsx("div", {
                    ...
                    children: y.map((t) => {
                      const s = M.filter((i) => i.carId === u && i.driverId === t.id).length,
                        n = A === t.id,
                        c =
                          Array.from(Z.values()).find(
                            (i) =>
                              i.driverName.toLowerCase().includes(t.name.toLowerCase()) ||
                              t.name.toLowerCase().includes(i.driverName.toLowerCase()),
                          )?.isOnline ?? !1,
                        o = c ? "Driving" : t.status || "Available",
                        d =
                          o === "Driving"
                        ...
                        "div",
                        {
                          ...
                          onDragStart: (i) => {
                            ((i.dataTransfer.effectAllowed = "move"),
                              i.dataTransfer.setData("text/plain", t.id),
                              i.dataTransfer.setData("driverId", t.id));
                          },
                          onClick: () => {
                            K(n ? null : t.id);
                          },
                          className: `p-2 bg-[#05070a]/50 border rounded-none hover:bg-[#11161d] flex items-center justify-between gap-2.5 group transition-all cursor-grab active:cursor-grabbing relative ${n ? "border-[#FFB800] bg-[#FFB800]/5" : "border-[#1c2430]"}`,
                          title:
                          ...
                          children: [
                            e.jsxs("div", {
                              ...
                              children: [
                                e.jsx("button", {
                                  type: "button",
                                  onClick: (i) => {
                                    (i.stopPropagation(), K(n ? null : t.id));
                                  },
                                  className:
                            ...
                            e.jsxs("div", {
                              ...
                              children: [
                                ...
                                e.jsx("button", {
                                  type: "button",
                                  onClick: (i) => {
                                    (i.stopPropagation(), mt(t.id));
                                  },
                                  className: `text-[7.5px] font-black uppercase tracking-widest px-1.5 py-0.5 border rounded-none font-mono shrink-0 cursor-pointer select-none hover:brightness-125 transition-all ${d}`,
                            ...
                            e.jsx("button", {
                              ...
                              title: "Evict driver",
                              children: e.jsx(be, { className: "w-2.5 h-2.5" }),
                            }),
                          ],
                        },
                        t.id,
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      ...
                      e.jsx("button", {
                        ...
                        onClick: () => {
                          p
                            ? (p.fuelBurnPerLap > 0 && Te(Number(p.fuelBurnPerLap.toFixed(2))),
                              p.lastLapSec > 0 && Ue(Math.round(p.lastLapSec)))
                            : a.connected && Te(2.85);
                        },
                        disabled: !p && !a.connected,
                        className:
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("input", {
                            type: "number",
                            value: W,
                            onChange: (t) => st(Number(t.target.value)),
                            className:
                              "w-full bg-[#05070a] border border-[#1c2430] rounded-none px-1.5 py-1 text-white font-mono font-bold text-[8.5px] focus:border-[#3B82F6] focus:outline-none",
                          }),
                        ],
                              ...
                              e.jsxs("div", {
                                children: [
                                  ...
                                  e.jsxs("span", {
                                    ...
                                    children: [Ie.totalFuel.toFixed(1), " L"],
                                  }),
                                ],
                              }),
                              e.jsxs("div", {
                        ...
                        "div",
                        {
                          ...
                          onDragStart: (s) => {
                            ((s.dataTransfer.effectAllowed = "move"),
                              s.dataTransfer.setData("text/plain", t.id),
                              s.dataTransfer.setData("driverId", t.id));
                          },
                          className:
                          ...
                          children: t.name.split(" ").slice(-1)[0],
                        },
                        t.id,
                      ),
                    ),
                              ...
                              [
                                ...
                                return e.jsx(
                                  "button",
                                  {
                                    ...
                                    className: `px-2 py-0.5 cursor-pointer rounded-none border-0 transition-all font-mono font-bold ${s ? "bg-[#1c2430] text-white" : `${t.color}`}`,
                                    children: t.label,
                                  },
                                  t.id,
                                );
                              ...
                              ["24h", "6h", "1h", "15m"].map((t) => {
                                ...
                                return e.jsx(
                                  "button",
                                  {
                                    type: "button",
                                    onClick: () => ze(t),
                                    className: `px-1.8 py-0.5 cursor-pointer rounded-none border-0 transition-all font-mono font-bold ${s ? "bg-[#1c2430] text-white" : "text-[#7a828c] hover:text-white"}`,
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsxs("span", {
                                ...
                                children: [String(N).padStart(2, "0"), ":00:00"],
                              }),
                            ],
                          }),
                          e.jsx("div", { className: "w-px h-5 bg-[#1c2430]" }),
                          e.jsxs("div", {
                            className: "text-right",
                            children: [
                              e.jsx("span", {
                                        ...
                                        onClick: () => {
                                          (q(t),
                                            t >= 12
                                              ? (v("24h"), D(1), L(0))
                                              : t >= 4 || t >= 2
                                                ? (v("6h"), D(Math.max(1, t / 6)), L(0))
                                                : (v("1h"), D(Math.max(1, t / 1)), L(0)));
                                        },
                                        className: `flex-1 text-[8px] py-0.5 text-center font-bold border-0 cursor-pointer transition-colors ${s ? "bg-[#FFB800] text-black" : "text-[#7a828c] hover:text-white"}`,
                                        children: [t, "H"],
                                      },
                                      t,
                                    );
                                      ...
                                      "div",
                                      {
                                        ...
                                        children: [
                                          e.jsx("span", {
                                            ...
                                            style: {
                                              backgroundColor: t.state ? t.color : "#3A3F47",
                                            },
                                          }),
                                          e.jsx("span", {
                                            className: t.state
                            ...
                            e.jsxs("div", {
                              ...
                              children: [
                                e.jsxs("div", {
                                  ...
                                  children: [
                                    e.jsx("span", { children: "RACE DURATION:" }),
                                    e.jsxs("span", {
                                ...
                                e.jsxs("div", {
                                  className:
                                    "flex justify-between border-b border-[#1c2430]/50 pb-0.5",
                                  children: [
                                    e.jsx("span", { children: "ACTIVE CO-PILOTS:" }),
                                    e.jsxs("span", {
                                      className: "text-white font-bold",
                                      children: [y.length, " REGISTERED"],
                                    }),
                                  ],
                                }),
                                e.jsxs("div", {
                                  className:
                                    "flex justify-between border-b border-[#1c2430]/50 pb-0.5",
                                  children: [
                                    e.jsx("span", { children: "RELAY PIPELINE:" }),
                                    e.jsx("span", {
                                      className: `font-bold ${J ? "text-[#00D17F]" : "text-red-400"}`,
                                ...
                                e.jsxs("div", {
                                  ...
                                  children: [
                                    e.jsx("span", { children: "DB ROW SEGREGATION:" }),
                                    e.jsx("span", {
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsx("div", {
                        ...
                        children: Array.from({ length: 7 }).map((t, s) =>
                          e.jsx(
                            "div",
                            {
                              className:
                            ...
                            ? e.jsxs("div", {
                                ...
                                children: [
                                  ...
                                  e.jsx("div", {
                                    ...
                                    children: pt.map((t, s) => {
                                      const n = y.find((i) => i.id === t.driverId),
                                        l = A && t.driverId !== A,
                                        c = A && t.driverId === A,
                                        o = x === s,
                                        d = b(s);
                                            ...
                                            "div",
                                            {
                                              ...
                                              onDrop: (i) => {
                                                const j =
                                                  ...
                                                  i.dataTransfer.getData("driverId");
                                                j && Ve(s, j);
                                              },
                                              onClick: () => {
                                                if (y.length === 0) return;
                                                const i = y.findIndex((T) => T.id === t.driverId);
                                                let j = "";
                                                (t.driverId === ""
                                                  ? (j = y[0].id)
                                                  : i === y.length - 1
                                              ...
                                              onMouseEnter: () => f(s),
                                              onMouseLeave: () => f(null),
                                              className: `rounded-none border relative flex flex-col justify-center items-center px-1.5 select-none overflow-hidden transition-all duration-150 group/slot cursor-pointer ${n ? "bg-gradient-to-b from-[#11161d] to-[#0b0f14] shadow-md" : "bg-[#05070a] border-dashed border-[#1c2430] hover:bg-[#11161d]/40"} ${l ? "opacity-25" : "opacity-100"} ${c ? "ring-1 ring-[#FFB800]" : ""} ${o ? "brightness-125" : ""}`,
                                              style: {
                                                ...d,
                                                left: `calc(${parseFloat(d.left)}% + 2px)`,
                                                width: `calc(${parseFloat(d.width)}% - 4px)`,
                                                borderColor: n ? n.color : "#1C2430",
                                              },
                                              title: n
                                                ? `Driver: ${n.name}
${t.note}. Hover to highlight fuel curve. Click to cycle drivers.`
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsxs("div", {
                                ...
                                children: [
                                  e.jsxs("div", {
                                    className: `font-black flex items-center justify-center tracking-widest uppercase font-rajdhani ${je ? "bg-[#FF4D4D]/25 text-[#FF4D4D]" : "bg-black/40 text-[#7a828c] line-through decoration-red-500/50"}`,
                                    style: b(0),
                                    children: ["SOFT (S1) ", !je && "LOCKED"],
                                  }),
                                  e.jsxs("div", {
                                    className: `font-black flex items-center justify-center tracking-widest uppercase font-rajdhani ${Ne ? "bg-[#FFB800]/25 text-[#FFB800]" : "bg-black/40 text-[#7a828c] line-through decoration-amber-500/50"}`,
                                    style: b(1),
                                    children: ["MEDIUM (S2) ", !Ne && "LOCKED"],
                                  }),
                                  e.jsxs("div", {
                                    className: `font-black flex items-center justify-center tracking-widest uppercase font-rajdhani ${ge ? "bg-[#E2E4E8]/15 text-[#E2E4E8]/70" : "bg-black/40 text-[#7a828c] line-through decoration-white/50"}`,
                                    style: b(2),
                                    children: ["HARD (S3) ", !ge && "LOCKED"],
                                  }),
                                  e.jsxs("div", {
                                    className: `font-black flex items-center justify-center tracking-widest uppercase font-rajdhani ${ve ? "bg-[#3B82F6]/25 text-[#3B82F6]" : "bg-black/40 text-[#7a828c] line-through decoration-blue-500/50"}`,
                                    style: b(3),
                                    children: ["WET (S4) ", !ve && "LOCKED"],
                                  }),
                                ],
                              }),
                            ],
                                ...
                                children: [
                                  ...
                                  e.jsxs("div", {
                                    ...
                                    children: [
                                      ...
                                      e.jsx("span", { className: "text-white", children: "2.82L" }),
                                    ],
                                  }),
                                  e.jsxs("div", {
                                    className:
                          ...
                          e.jsxs("div", {
                            className: `flex items-center relative py-0.5 transition-all duration-200 ${m === "fcy" ? "bg-yellow-500/5 ring-1 ring-yellow-500/30" : ""}`,
                            children: [
                              e.jsx("div", {
                                className: "w-24 shrink-0 pr-2",
                                children: e.jsx("span", {
                                  className: `text-[7px] font-bold tracking-widest uppercase font-rajdhani transition-colors ${m === "fcy" ? "text-[#FFB800]" : "text-[#7a828c]"}`,
                              ...
                              e.jsxs("div", {
                                ...
                                children: [
                                  ...
                                  e.jsx("div", {
                                    className: `bg-red-500/20 flex items-center justify-center font-bold text-red-400 tracking-wider font-rajdhani ${m === "fcy" ? "animate-pulse border-red-500" : ""}`,
                                    style: b(1),
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsxs("div", {
                                className: `flex-1 relative transition-all duration-300 rounded-none border border-[#1c2430] bg-[#0b0f14]/50 p-[1px] overflow-hidden ${m === "wet" ? "h-10" : "h-4"}`,
                                children: [
                                  e.jsxs("div", {
                                    className:
                                    ...
                                    style: b(0),
                                    children: [
                                      e.jsx("span", { children: "100% DRY / SUNNY" }),
                                      m === "wet" &&
                                        e.jsx("span", {
                                  ...
                                  e.jsxs("div", {
                                    ...
                                    style: b(1),
                                    children: [
                                      e.jsx("span", { children: "40% DAMP OVERCAST" }),
                                      m === "wet" &&
                                        e.jsx("span", {
                                  ...
                                  e.jsxs("div", {
                                    ...
                                    style: b(2),
                                    children: [
                                      e.jsx("span", { children: "65% WET TRANSITION" }),
                                      m === "wet" &&
                                        e.jsx("span", {
                                  ...
                                  e.jsxs("div", {
                                    ...
                                    style: b(3),
                                    children: [
                                      e.jsx("span", { children: "80% WET STORM WARNING" }),
                                      m === "wet" &&
                                        e.jsx("span", {
                            ...
                            children: [
                              e.jsx("div", {
                                ...
                                children: e.jsxs("span", {
                                  ...
                                  children: [
                                    e.jsx("span", { className: "w-1 h-1 bg-[#7a828c] opacity-50" }),
                                    "RIVAL GHOST",
                                  ],
                                }),
                              }),
                      ...
                      e.jsx("div", {
                        ...
                        children: [
                          ...
                          return e.jsx(
                            ...
                            {
                              onClick: () => k(t.id),
                              className: `px-2.5 py-0.5 text-[8px] font-black uppercase tracking-widest cursor-pointer transition-all rounded-none border-0 ${s ? "bg-[#1c2430] text-white" : "text-[#7a828c] hover:text-[#E2E4E8] hover:bg-[#11161d]/50"}`,
                              children: t.label,
                            },
                            t.id,
                          );
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          e.jsxs("div", {
                            ...
                            children: [
                              e.jsx("span", { children: "100% / MAX" }),
                              e.jsx("span", { children: "ESTIMATED DOCK" }),
                            ],
                          }),
                          e.jsxs("div", {
                            className: "flex justify-between border-b border-[#1c2430]/10 pb-0.5",
                            children: [
                              ...
                              e.jsx("span", { children: "STINT LAP LIMIT" }),
                            ],
                          }),
                          e.jsxs("div", {
                            className: "flex justify-between border-b border-[#1c2430]/10 pb-0.5",
                            children: [
                              ...
                              e.jsx("span", { children: "PIT RECHARGE" }),
                            ],
                          }),
                          e.jsxs("div", {
                            className: "flex justify-between border-b border-[#1c2430]/10 pb-0.5",
                            children: [
                              e.jsx("span", { children: "25% / MIN" }),
                              e.jsx("span", { children: "CRITICAL LIMIT" }),
                            ],
                          }),
                          e.jsx("div", {
                            ...
                            children: Array.from({ length: 7 }).map((t, s) => {
                              ...
                              return N >= 4 && P < 4
                                ? s === 0
                                  ? e.jsx("span", { children: "T+0:00" }, s)
                                  : s === 6
                                    ? e.jsx("span", { children: "FINISH" }, s)
                                    : e.jsxs(
                                        "span",
                                        { children: [c, ":", String(o).padStart(2, "0")] },
                                        s,
                                      )
                                : s === 0
                                  ? e.jsx("span", { children: "T+0 MIN" }, s)
                                  : e.jsxs(
                                      "span",
                                      { children: [c > 0 ? `${c}H ` : "", "$", o, "M"] },
                                      s,
                                    );
                            }),
                          }),
                          ...
                          children: [
                            ...
                            w === "fuel" &&
                              e.jsxs(e.Fragment, {
                                children: [
                                  e.jsx("path", {
                                    d: R("M 5,20 Q 75,70 138,100"),
                                    fill: "none",
                                    stroke: "#3B82F6",
                                    strokeWidth: x === 0 ? "3" : "1.8",
                                    opacity: x !== null && x !== 0 ? "0.25" : "1",
                                    className: "transition-all duration-150",
                                  }),
                                  e.jsx("path", {
                                    d: R("M 140,20 Q 210,65 273,95"),
                                    fill: "none",
                                    stroke: "#3B82F6",
                                    strokeWidth: x === 1 ? "3" : "1.8",
                                    opacity: x !== null && x !== 1 ? "0.25" : "1",
                                    className: "transition-all duration-150",
                                  }),
                                  e.jsx("path", {
                                    d: R("M 275,20 Q 345,72 408,102"),
                                    fill: "none",
                                    stroke: "#3B82F6",
                                    strokeWidth: x === 2 ? "3" : "1.8",
                                    opacity: x !== null && x !== 2 ? "0.25" : "1",
                                    className: "transition-all duration-150",
                                  }),
                                  e.jsx("path", {
                                    d: R("M 410,20 Q 500,55 595,78"),
                                    fill: "none",
                                    stroke: "#3B82F6",
                                    strokeWidth: x === 3 ? "3" : "1.8",
                                    opacity: x !== null && x !== 3 ? "0.25" : "1",
                                    className: "transition-all duration-150",
                                  }),
                                  e.jsx("line", {
                                    x1: h(138),
                            ...
                            w === "tyre" &&
                              e.jsxs(e.Fragment, {
                                children: [
                                  e.jsx("path", {
                                    d: R("M 5,30 C 40,32 90,65 138,85"),
                                    fill: "none",
                                    stroke: "#00D17F",
                                    strokeWidth: x === 0 ? "3" : "1.8",
                                    opacity: x !== null && x !== 0 ? "0.25" : "1",
                                  ...
                                  e.jsx("path", {
                                    d: R("M 140,30 C 175,32 225,60 273,78"),
                                    fill: "none",
                                    stroke: "#00D17F",
                                    strokeWidth: x === 1 ? "3" : "1.8",
                                    opacity: x !== null && x !== 1 ? "0.25" : "1",
                                  ...
                                  e.jsx("path", {
                                    d: R("M 275,30 C 310,32 360,70 408,90"),
                                    fill: "none",
                                    stroke: "#00D17F",
                                    strokeWidth: x === 2 ? "3" : "1.8",
                                    opacity: x !== null && x !== 2 ? "0.25" : "1",
                                  ...
                                  e.jsx("path", {
                                    d: R("M 410,30 C 460,32 530,55 595,72"),
                                    fill: "none",
                                    stroke: "#00D17F",
                                    strokeWidth: x === 3 ? "3" : "1.8",
                                    opacity: x !== null && x !== 3 ? "0.25" : "1",
                            ...
                            w === "temp" &&
                              e.jsxs(e.Fragment, {
                                children: [
                                  e.jsx("path", {
                                    d: R("M 5,80 Q 150,40 300,55 T 600,90"),
                                    fill: "none",
                                    stroke: "#FFB800",
                                    strokeWidth: "1.8",
                  ...
                  children: [
                    e.jsx("thead", {
                      children: e.jsxs("tr", {
                        ...
                        children: [
                          e.jsx("th", { className: "px-2 py-1", children: "SYS_ID" }),
                          e.jsx("th", {
                            className: "px-2 py-1",
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("div", {
                        ...
                        children: [
                          e.jsx("span", {
                            className: `size-1.5 rounded-full ${J ? "bg-[#00D17F] shadow-[0_0_6px_#00D17F]" : "bg-red-500 animate-pulse shadow-[0_0_6px_rgba(239,68,68,0.7)]"}`,
                          }),
                          e.jsx("span", { children: "ACTIVE MONITOR CHANNEL HUD" }),
                        ],
                      }),
                      e.jsxs("div", {
                        ...
                        children: [
                          e.jsx("button", {
                            type: "button",
                            onClick: () => te(!0),
                            className: `text-[8px] border px-1.5 py-0.5 uppercase tracking-widest font-mono font-bold transition-all rounded-none cursor-pointer ${S ? "text-[#00D17F] bg-[#00D17F]/10 border-[#00D17F]/30 hover:bg-[#00D17F]/20" : "text-[#3B82F6] bg-[#3B82F6]/10 border-[#3B82F6]/30 hover:bg-[#3B82F6]/20"}`,
                            children: S ? `🔗 ${S}` : "+ JOIN TEAM",
                          }),
                          p &&
                            e.jsx("span", {
                              className:
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("div", {
                        ...
                        children: [
                          e.jsx("span", {
                            ...
                            children: p ? p.driverName : "L. VANTHOOR",
                          }),
                          e.jsx("span", {
                            className:
                              "font-mono text-white bg-[#11161d] border border-[#1c2430] px-1 rounded-none text-[8.5px]",
                            children:
                              p && p.carOperationalState?.sequenceId
                                ? `LAP ${p.carOperationalState.sequenceId}`
                                : "LAP 68",
                          }),
                        ],
                      }),
                      e.jsxs("div", {
                        ...
                        children: [
                          e.jsx("span", {
                            children: F ? F.name.toUpperCase() : "PORSCHE 963 LMDH",
                          }),
                          e.jsxs("span", {
                            className: "text-[#3B82F6] font-sans font-bold",
                            children: ["WEC ", F ? F.carClass : "GTP", " CLASS"],
                          }),
                        ],
                      }),
                    ],
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsxs("span", {
                            ...
                            children: [
                              p ? p.speedKph : a.connected ? a.speedKph : 243,
                              " ",
                              e.jsx("span", {
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("span", {
                            ...
                            children: p ? p.rpm : a.connected ? a.rpm : 7850,
                          }),
                        ],
                      }),
                    ],
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsx("span", {
                                ...
                                children: a.connected ? `${(a.throttle * 100).toFixed(0)}%` : "78%",
                              }),
                            ],
                          }),
                          e.jsx("div", {
                            ...
                            children: e.jsx("div", {
                              ...
                              style: { width: a.connected ? `${a.throttle * 100}%` : "78%" },
                            }),
                          }),
                        ],
                      }),
                        ...
                        children: [
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsx("span", {
                                className: "text-white font-bold text-[7.5px]",
                                children: a.connected ? `${(a.brake * 100).toFixed(0)}%` : "12%",
                              }),
                            ],
                          }),
                          e.jsx("div", {
                            ...
                            children: e.jsx("div", {
                              ...
                              style: { width: a.connected ? `${a.brake * 100}%` : "12%" },
                            }),
                          }),
                        ],
                      }),
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("div", {
                        ...
                        children: [
                          e.jsx("span", { children: "TRACK SECTOR" }),
                          e.jsx("span", { children: "DURATION" }),
                          e.jsx("span", { children: "DELTA" }),
                        ],
                      }),
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("span", { className: "text-white font-bold", children: "28.451" }),
                          e.jsx("span", {
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("span", { className: "text-white font-bold", children: "33.782" }),
                          e.jsx("span", {
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("span", { className: "text-white font-bold", children: "42.779" }),
                          e.jsx("span", {
                            className: "text-red-400 font-bold font-sans",
                            children: "+0.089",
                          }),
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("span", {
                            ...
                            children: p && p.deltaSec ? `+${p.deltaSec.toFixed(3)}s` : "+0.842s",
                          }),
                        ],
                      }),
                    ],
                ...
                children: [
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      ...
                      e.jsx(de, { data: le.speed, color: "#3B82F6" }),
                      e.jsxs("span", {
                        className: "text-white font-bold font-mono w-14 text-right",
                        children: [p ? p.speedKph : a.connected ? a.speedKph : 243, " km/h"],
                      }),
                    ],
                  }),
                  e.jsxs("div", {
                    ...
                    children: [
                      ...
                      e.jsx(de, { data: le.throttle, color: "#00D17F" }),
                      e.jsx("span", {
                        className: "text-white font-bold font-mono w-14 text-right",
                        children: a.connected ? `${(a.throttle * 100).toFixed(0)}%` : "78%",
                      }),
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      ...
                      e.jsx(de, { data: le.brake, color: "#FF4D4D" }),
                      e.jsx("span", {
                        className: "text-white font-bold font-mono w-14 text-right",
                        children: a.connected ? `${(a.brake * 100).toFixed(0)}%` : "12%",
                      }),
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      ...
                      e.jsx(de, { data: le.rpm, color: "#FFB800" }),
                      e.jsxs("span", {
                        className: "text-white font-bold font-mono w-14 text-right",
                        children: [p ? p.rpm : a.connected ? a.rpm : 7850, " rpm"],
                      }),
                    ],
                  }),
                  e.jsxs("div", {
                    ...
                    children: [
                      ...
                      e.jsx(de, { data: le.fuel, color: "#8B5CF6" }),
                      e.jsxs("span", {
                        className: "text-white font-bold font-mono w-14 text-right",
                        children: [
                          p
                            ? p.fuelRemainingL.toFixed(1)
                            : a.connected
                              ? a.fuelRemainingL.toFixed(1)
                              : "54.2",
                          " L",
                        ],
                      }),
          ...
          e.jsxs("div", {
            ...
            children: [
              e.jsxs("span", {
                ...
                children: [
                  e.jsx("span", { className: "size-1 bg-[#00D17F] rounded-full" }),
                  "6 TEAM STATUS",
                ],
              }),
              e.jsxs("div", {
              ...
              e.jsxs("div", {
                ...
                children: [
                  e.jsx("span", { className: "text-[#7a828c] uppercase", children: "TRACK GRIP" }),
                  e.jsx("span", {
                    ...
                    "div",
                    {
                      ...
                      children: [
                        e.jsxs("div", {
                          className: "flex items-center gap-1.5",
                          children: [
                            e.jsx("span", { className: `w-1.5 h-1.5 rounded-full ${s}` }),
                            e.jsx("span", {
                ...
                children: [
                  e.jsxs("svg", {
                    ...
                    children: [
                      ...
                      e.jsx("circle", { cx: "28", cy: "28", r: "2", fill: "#00D17F" }),
                      e.jsx("circle", {
                        cx: "71",
                        cy: "28",
                        r: "5",
                      ...
                      e.jsx("circle", { cx: "71", cy: "28", r: "2", fill: "#00D17F" }),
                      e.jsx("line", {
                        x1: "90",
                        y1: "10",
                        x2: "98",
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("div", {
                        className: "flex justify-between",
                        children: [
                          e.jsx("span", { children: "PUNCTURE PROB" }),
                          e.jsx("span", {
                            className: "text-[#00D17F] font-bold",
                      ...
                      e.jsxs("div", {
                        className: "flex justify-between",
                        children: [
                          e.jsx("span", { children: "BRAKE THERMAL" }),
                          e.jsx("span", {
                            className: "text-[#00D17F] font-bold",
                      ...
                      e.jsxs("div", {
                        className: "flex justify-between",
                        children: [
                          e.jsx("span", { children: "HYBRID REGEN DEFICIT" }),
                          e.jsx("span", {
                            className: "text-white font-bold",
                            children: "-0.12 kW",
                          }),
                      ...
                      e.jsxs("div", {
                        className: "flex justify-between",
                        children: [
                          e.jsx("span", { children: "OVERHEAT COEFF" }),
                          e.jsx("span", {
                            className: "text-[#00D17F] font-bold",
                      ...
                      e.jsxs("div", {
                        className: "flex justify-between",
                        children: [
                          e.jsx("span", { children: "TYRE SLIP LIMIT" }),
                          e.jsx("span", {
                ...
                children: [
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("span", {
                        children: [
                          e.jsx("span", { className: "text-white", children: "14:32:15" }),
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("span", {
                        children: [
                          e.jsx("span", { className: "text-[#3B82F6]", children: "14:31:48" }),
                          " CAR #7 ENTERED PITS",
                        ],
                      }),
                      e.jsx("span", {
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("span", {
                        children: [
                          e.jsx("span", { className: "text-[#FFB800]", children: "14:30:22" }),
                  ...
                  e.jsxs("div", {
                    ...
                    children: [
                      e.jsxs("span", {
                        children: [
                          e.jsx("span", { className: "text-[#00D17F]", children: "14:28:11" }),
                  ...
                  e.jsx("div", {
                    ...
                    children: S
                      ...
                      : e.jsxs("form", {
                          onSubmit: (t) => {
                            (t.preventDefault(),
                              Fe.trim() && (ye(Fe.trim().toUpperCase()), te(!1)));
                          },
                          className: "space-y-4",
                          children: [
                            e.jsxs("div", {
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("input", {
                            ...
                            type: "text",
                            value: G.name,
                            onChange: (t) => he((s) => ({ ...s, name: t.target.value })),
                            className:
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsx("input", {
                                ...
                                type: "text",
                                value: G.number,
                                onChange: (t) => he((s) => ({ ...s, number: t.target.value })),
                                className:
                                  "w-full bg-[#05070a] border border-[#1c2430] rounded-none px-3 py-2 text-xs font-mono font-bold text-[#E2E4E8] focus:border-[#3B82F6] focus:outline-none",
                                placeholder: "7",
                              }),
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsxs("select", {
                                id: "car-cls",
                                value: G.carClass,
                                onChange: (t) => he((s) => ({ ...s, carClass: t.target.value })),
                                className:
                                  "w-full bg-[#05070a] border border-[#1c2430] rounded-none px-3 py-2 text-xs font-mono font-bold text-[#E2E4E8] focus:border-[#3B82F6] focus:outline-none",
                                children: [
                                  e.jsx("option", { value: "GT3", children: "GT3" }),
                                  e.jsx("option", { value: "GTP", children: "GTP" }),
                                  e.jsx("option", { value: "LMP2", children: "LMP2" }),
                                ],
                              }),
                            ],
                          }),
                      ...
                      e.jsxs("div", {
                        ...
                        children: [
                          ...
                          e.jsx("input", {
                            ...
                            type: "text",
                            value: $.name,
                            onChange: (t) => me((s) => ({ ...s, name: t.target.value })),
                            className:
                              "w-full bg-[#05070a] border border-[#1c2430] rounded-none px-3 py-2 text-xs font-mono font-bold text-[#E2E4E8] focus:border-[#3B82F6] focus:outline-none",
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsx("input", {
                                ...
                                type: "text",
                                value: $.shortName,
                                onChange: (t) => me((s) => ({ ...s, shortName: t.target.value })),
                                className:
                                  "w-full bg-[#05070a] border border-[#1c2430] rounded-none px-3 py-2 text-xs font-mono font-bold text-[#E2E4E8] focus:border-[#3B82F6] focus:outline-none",
                                placeholder: "VAN",
                              }),
                          ...
                          e.jsxs("div", {
                            ...
                            children: [
                              ...
                              e.jsx("input", {
                                ...
                                type: "color",
                                value: $.color,
                                onChange: (t) => me((s) => ({ ...s, color: t.target.value })),
                                className:
