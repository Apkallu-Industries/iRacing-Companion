...
async function ensureNodeExe() {
  ...
  console.log(
    `[copy-bridge] 🌐 Downloading portable Node.js v${nodeVersion} for Windows x64 from ${url}...`,
  );
  ...
  try {
    ...
    console.warn(
      `[copy-bridge] ⚠️ Pure Node.js download failed: ${err.message}. Trying PowerShell fallback...`,
    );
    try {
      const { execSync } = require("child_process");
      execSync(
        `powershell.exe -NoProfile -Command "Invoke-WebRequest -Uri '${url}' -OutFile '${nodeExePath}' -UseBasicParsing"`,
        { stdio: "inherit" },
      );
      console.log(
        "[copy-bridge] ✅ Successfully downloaded portable node.exe via PowerShell fallback!",
      );
    } catch (psErr) {
      console.error(
        "[copy-bridge] ❌ Failed to download portable node.exe via all methods:",
        psErr.message,
      );
      process.exit(1);
    }
  ...
  try {
    ...
    if (fs.existsSync(viteServer)) {
      ...
      try {
        ...
        console.log(
          `[copy-bridge] ✅ Wrote ${path.join(bridgeServer, "package.json")} with type: module`,
        );
      } catch (err) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      if (forceInstall) {
        ...
        console.log(
          "[copy-bridge] ⚡ Skipping bridge npm install (node_modules already present). Use --force-install to force.",
        );
      }
    }
